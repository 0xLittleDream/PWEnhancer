...
if (chrome && chrome.storage) {
    ...
    // Listen for storage changes (when popup toggles are clicked)
    ...
    // Listen for direct messages for instant speed apply
...
function injectTimer() {
    ...
    // Check if we already injected the timer to prevent duplicates
    if (document.getElementById('pw-truetime-timer')) {
        // If it exists, make sure we occasionally enforce custom speed on navigation
        if (video && currentSettings.customSpeed !== 1.0 && video.playbackRate !== currentSettings.customSpeed) {
            // Uncomment to force speed on every navigation: applyCustomSpeed(video);
        }
        return true;
    }
    ...
    divider2.style.cssText = "width: 1px; height: 16px; background: rgba(255,255,255,0.2); margin: 0 4px;";

    // Assemble Pill
    timerContainer.appendChild(timeSection);
    timerContainer.appendChild(divider);
    timerContainer.appendChild(speedSection);
    timerContainer.appendChild(divider2);
    timerContainer.appendChild(captureSection);
    ...
    const setSpeed = (newSpeed) => {
        ...
        chrome.storage.sync.set({ customSpeed: newSpeed });
...
// --- STUDY TIME TRACKER ---
setInterval(() => {
    // Only count as studying if the tab is visible and actively focused
    if (!document.hidden && document.hasFocus()) {
        const url = window.location.href;
        if (url.includes('/practice-v2') || url.includes('/study-v2/notes') || url.includes('/watch')) {
